...
const Header = () => {
  ...
  return (
    <motion.header
      ...
      animate={{
        boxShadow: "0 10px 15px -3px rgba(18, 128, 253, 1)",
    ...
    >
      <nav className="flex justify-between items-center pt-4 pb-2">
        <div className="flex flex-1 pl-8 font-extrabold text-2xl sm:text-[2rem] text-primary">
          <a href="#">
            <h3>
              Vict<i className="fas fa-circle-half-stroke rotate-90"></i>r
            </h3>
          </a>
        </div>
        <ul className="hidden sm:flex flex-auto justify-around items-center">
          {menuLinks.map((item, index) => {
            return (
              <li className="flex font-bold hover:text-secondary" key={index}>
                <a href={`#${item}`}>{item}</a>
              </li>
            );
          })}
        ...
        <i
          className="fas fa-bars sm:hidden pr-4 text-2xl"
