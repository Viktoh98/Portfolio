...
const Project = () => {
    ...
    // const Projects = data.projects_set

    return (
        <section id="Projects" className="pt-32 pb-12">
            <motion.div
                className="pb-16 text-center flex justify-center items-center flex-col">
                <h2 className="font-bold text-4xl md:text-6xl text-primary">Projects</h2>
                <motion.hr 
                    initial = {{scaleX: 0}}
        ...
        <div className="py-6 grid items-center justify-center grid-row sm:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-5 sm:justify-around px-8 overflow-hidden">
                {Projects.map((project, index) => <Frame key={project.id} {...project} delay={index} />)}
            </div>
        </section>
    )
}
